...
class TimePickerView extends ConstraintLayout implements TimePickerControls {
        ...
        this.selectionListener = new View.OnClickListener() {
            public void onClick(View v) {
                if (TimePickerView.this.onSelectionChangeListener != null) {
                    TimePickerView.this.onSelectionChangeListener.onSelectionChanged(((Integer) v.getTag(R.id.f1841selection_type)).intValue());
                }
            }
        };
        LayoutInflater.from(context).inflate(R.layout.f1944material_timepicker, this);
        this.clockFace = (ClockFaceView) findViewById(R.id.f1774material_clock_face);
        MaterialButtonToggleGroup materialButtonToggleGroup = (MaterialButtonToggleGroup) findViewById(R.id.f1778material_clock_period_toggle);
        this.toggle = materialButtonToggleGroup;
        materialButtonToggleGroup.addOnButtonCheckedListener(new MaterialButtonToggleGroup.OnButtonCheckedListener() {
            public void onButtonChecked(MaterialButtonToggleGroup group, int checkedId, boolean isChecked) {
                int i = checkedId == R.id.f1777material_clock_period_pm_button ? 1 : 0;
                if (TimePickerView.this.onPeriodChangeListener != null && isChecked) {
                    TimePickerView.this.onPeriodChangeListener.onPeriodChange(i);
                }
            }
        });
        this.minuteView = (Chip) findViewById(R.id.f1783material_minute_tv);
        this.hourView = (Chip) findViewById(R.id.f1780material_hour_tv);
        this.clockHandView = (ClockHandView) findViewById(R.id.f1775material_clock_hand);
        setupDoubleTap();
        setUpDisplay();
    }

    private void setUpDisplay() {
        this.minuteView.setTag(R.id.f1841selection_type, 12);
        this.hourView.setTag(R.id.f1841selection_type, 10);
    ...
    private void updateToggleConstraints() {
        if (this.toggle.getVisibility() == 0) {
            ...
            constraintSet.clear(R.id.f1773material_clock_display, i);
    ...
    public void updateTime(int period, int hourOfDay, int minute) {
        this.toggle.check(period == 1 ? R.id.f1777material_clock_period_pm_button : R.id.f1776material_clock_period_am_button);
